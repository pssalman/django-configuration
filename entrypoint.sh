...
export DJANGO_SETTINGS_MODULE=conf.settings.${ENVIRONMENT}
...
exec gunicorn conf.wsgi:application \
    ...
    --workers 3 \
    --log-level=info \
    --log-file=./logs/gunicorn/gunicorn-${ENVIRONMENT}.log \
    --access-logfile=./logs/gunicorn/gunicorn-access-${ENVIRONMENT}.log